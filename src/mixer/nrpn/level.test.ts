...
describe('LevelNRPNCalculator', () => {
	...
	type LevelOK = { type: 'ok'; result: Param }
	...
	const tests = {
		...
		fxReturn: {
			...
			//     SQ-5, remove?
			group: [
				[0, 0, { type: 'ok', result: { MSB: 0x4b, LSB: 0x34 } }],
				[0, 11, { type: 'ok', result: { MSB: 0x4b, LSB: 0x3f } }],
				[2, 3, { type: 'ok', result: { MSB: 0x4b, LSB: 0x4f } }],
				[6, 1, { type: 'ok', result: { MSB: 0x4b, LSB: 0x7d } }],
				[4, 7, { type: 'ok', result: { MSB: 0x4b, LSB: 0x6b } }],
				[8, 0, { type: 'error', match: 'fxReturn=8 is invalid' }],
